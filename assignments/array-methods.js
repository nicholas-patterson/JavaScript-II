...
runners.map(function(firstName) {
  let name = firstName.first_name.toUpperCase();
  allCaps.push(name);
});
console.log(allCaps);
...
runners.filter(function(large) {
  if (large.shirt_size === "L") {
    let people = `${large.first_name} ${large.last_name}`;
    largeShirts.push(people);
  }
});
...
// Problem 1
//Get All runners email addresses.
let email = [];
...
// Problem 2
//Company each runner represents
let company = [];
runners.filter(function(runner) {
  item = runner.company_name;
  company.push(item);
});
console.log(company);

// Problem 3
// Create new array last name only
let lastName = [];
runners.map(function(runner) {
  item = runner.last_name;
  lastName.push(item);
});
